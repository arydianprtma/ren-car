...
function getStatusLabel($status) {    switch ($status) {        case 'menunggu':            return '<span class="bg-yellow-100 text-yellow-800 px-3 py-1 rounded-full text-xs font-medium">Menunggu Pembayaran</span>';        case 'dibayar':            return '<span class="bg-green-100 text-green-800 px-3 py-1 rounded-full text-xs font-medium">Dibayar</span>';        case 'dikonfirmasi':            return '<span class="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-xs font-medium">Menunggu Konfirmasi</span>';        case 'berjalan':            return '<span class="bg-green-100 text-green-800 px-3 py-1 rounded-full text-xs font-medium">Berjalan</span>';        case 'selesai':            return '<span class="bg-green-100 text-green-800 px-3 py-1 rounded-full text-xs font-medium">Selesai</span>';        case 'dibatalkan':            return '<span class="bg-red-100 text-red-800 px-3 py-1 rounded-full text-xs font-medium">Dibatalkan</span>';        default:            return '<span class="bg-gray-100 text-gray-800 px-3 py-1 rounded-full text-xs font-medium">' . ucfirst(str_replace('_', ' ', $status ?? '')) . '</span>';    }
}
?>
...
<div class="bg-gray-100 py-3">
    <div class="container mx-auto px-6">
        <div class="flex text-sm">
            ...
            <span class="text-gray-600">Pemesanan Saya</span>
        </div>
    </div>
</div>
...
<section class="py-12 bg-white">
    <div class="container mx-auto px-6">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Pemesanan Saya</h1>
        ...
        <div class="bg-white rounded-xl shadow-sm p-4 border border-gray-200 mb-6">
            <form action="" method="GET" class="flex flex-wrap items-center gap-4">
                ...
                <div class="flex items-end">
                    ...
                    <?php if (!empty($status)): ?>
                    <a href="<?= USER_URL ?>pemesanan.php" class="ml-2 text-blue-600 hover:text-blue-800 flex items-center">
        ...
        <?php if (empty($pemesananList)): ?>
            <div class="text-center py-16 bg-white rounded-xl shadow-md hover:shadow-lg transition-all duration-300 border border-gray-200">
                <div class="flex flex-col items-center justify-center">
                    <div class="w-24 h-24 flex items-center justify-center rounded-full bg-blue-100 text-blue-500 mb-6">
                        <i class="fas fa-calendar-alt text-4xl"></i>
                    </div>
                    <h3 class="text-xl font-bold text-gray-800 mb-3">Belum Ada Pemesanan</h3>
                    ...
                    <a href="<?= USER_URL ?>mobil.php" class="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition-all inline-block shadow-md hover:shadow-lg">
                ...
                <?php foreach ($pemesananList as $pemesanan): ?>
                    ...
                    <div class="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-200 hover:shadow-md transition-all">
                        <div class="grid grid-cols-1 md:grid-cols-12">
                            <!-- Gambar Mobil -->
                            <div class="md:col-span-3 h-48 md:h-auto bg-gray-100 relative">
                            ...
                            <div class="md:col-span-9 p-6">
                                <div class="flex flex-col md:flex-row justify-between md:items-center mb-3">
                                    <h2 class="text-lg font-bold text-gray-800 mb-2 md:mb-0"><?= $pemesanan['merk'] ?> <?= $pemesanan['model'] ?></h2>
                                    <?= getStatusLabel($pemesanan['status_pemesanan']) ?>
                                </div>
                                
                                <p class="text-gray-600 text-sm mb-3">Kode Pemesanan: <span class="font-medium"><?= $pemesanan['kode_pemesanan'] ?></span></p>
                                
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
                                    <div>
                                        <p class="text-gray-600 text-sm">Tanggal Mulai: <span class="font-medium"><?= date('d F Y', strtotime($pemesanan['tanggal_mulai'])) ?></span></p>
                                        <p class="text-gray-600 text-sm">Tanggal Selesai: <span class="font-medium"><?= date('d F Y', strtotime($pemesanan['tanggal_selesai'])) ?></span></p>
                                        <p class="text-gray-600 text-sm">Durasi: <span class="font-medium"><?= $durasi ?> hari</span></p>
                                    </div>
                                    <div>
                                                                                <p class="text-gray-600 text-sm">Harga per Hari: <span class="font-medium">Rp <?= number_format($pemesanan['total_harga'] / $durasi, 0, ',', '.') ?></span></p>                                        <p class="text-gray-600 text-sm">Total Biaya: <span class="font-semibold text-blue-600">Rp <?= number_format($pemesanan['total_harga'], 0, ',', '.') ?></span></p>
                                        <p class="text-gray-600 text-sm">Tanggal Pesan: <span class="font-medium"><?= date('d F Y H:i', strtotime($pemesanan['created_at'])) ?></span></p>
                                    </div>
                                </div>
                                
                                <div class="flex flex-col md:flex-row justify-end space-y-2 md:space-y-0 md:space-x-2 pt-3 border-t border-gray-100">
                                    <?php if ($pemesanan['status_pemesanan'] === 'menunggu'): ?>
                                        <a href="<?= USER_URL ?>pemesanan_detail.php?kode=<?= $pemesanan['kode_pemesanan'] ?>" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-all text-center">
    ...
    <div class="container mx-auto px-6">
        <div class="bg-gradient-to-r from-blue-700 to-blue-500 rounded-xl p-8 shadow-lg relative overflow-hidden">
            ...
            <div class="relative z-10 text-center text-white">
                ...
                <a href="<?= USER_URL ?>mobil.php" class="bg-white text-blue-600 font-semibold py-2 px-6 rounded-lg hover:bg-blue-50 transition duration-300 inline-block shadow-md">
                    Lihat Katalog Mobil
                </a>
            </div>
        </div>
